...
Após a recepção de um pedido \emph{AuthnRequest}, o Autenticação.Gov processa a autenticação do utilizador e retorna o seguinte pedido como resposta.
...
\vspace{-3mm}
\subsection{Incongruências no Autenticação.Gov}
...
\begin{enumerate}
    ...
    Como acontece com o algoritmo de encriptação, a utilização de \emph{SSLv3} é desaconselhada, visto este ser considerado como \emph{deprecated}\footnote{Para mais informações aceder ao URL seguinte:  \url{https://tools.ietf.org/html/rfc7568}}. É importante referir que nesta dissertação foi utilizado um certificado \emph{SSLv3} devido à facilidade de obtenção do mesmo, sendo que a alteração para \emph{TLS} pode ser feita através da obtenção de um novo certificado, não sendo necessária qualquer reestruturação da solução já implementada.
    
    Em suma, é recomendado a criação de um período de transição, no qual os utilizadores do Autenticação.Gov possam alterar o certificado utilizado para uma vertente do \emph{TLS}, como por exemplo, o \emph{TLS 1.3}\footnote{Para mais informações aceder ao URL seguinte: \url{https://tools.ietf.org/html/rfc8446}}.
    
    \item \textbf{Definição de ID nos pedidos}
    
    Cada pedido de autenticação \emph{AuthnRequest} possui um identificador único, designaod por \emph{ID}. Este campo é gerado através do \emph{UUID\//v4}, sendo sempre gerados identificadores totalmente aleatórios.
    
    Por norma, estes identificadores gerados podem começar com qualquer letra ou número, sendo que durante o desenvolvimento da autenticação com o Cartão de Cidadão, apenas um número pequeno de pedidos de autenticação eram de facto gerados com sucesso. Isto devia-se ao caso de o \emph{ID} ter como primeiro caractere uma letra.
    
    Em suma, apenas gerava um pedido correto quando era iniciado por um número. A solução proposta pela \gls{ama} foi preceder todos os identificadores de um "\_", tornando todos os identificadores gerados válidos. 
...
Embora seja impossível retirar qualquer tipo de informação visível do \emph{token} exemplificado anteriormente, este contem dados extremamente úteis, tendo sido gerado através da seguinte forma:
...
\begin{itemize}
    ...
    Após o uso de \emph{Base64URL enconding} sobre o algoritmo \textbf{HS256}, a informação acima representada é transcrita sobre a forma da seguinte string:
    ...
    Embora a definição de \gls{jwt} apenas abranja os dois elementos previamente estudados, o terceiro elemento tem como função garantir que o \gls{jwt} não foi indevidamente alterado.
